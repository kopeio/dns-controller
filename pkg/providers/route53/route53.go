...
import (
	...
	"github.com/kopeio/dns-controller/pkg/providers"
...
func NewRoute53Provider() (*Route53Provider, error) {
	...
	p := &Route53Provider{
		r53Client:  r53Client,
		elbClients: make(map[string]*elb.ELB),
	}
	return p, nil
}

func (p *Route53Provider) getELBClient(region string) *elb.ELB {
	p.mutex.Lock()
	defer p.mutex.Unlock()

	elbClient, found := p.elbClients[region]
	if !found {
...
func (p *Route53Provider) getELBInfo(dnsName string) (*elb.LoadBalancerDescription, error) {
	...
	glog.V(2).Infof("Querying ELB for load balancer information for %q in region %q", elbName, region)
	request := &elb.DescribeLoadBalancersInput{
		LoadBalancerNames: []*string{
			&elbName,
		},
	}
	elbClient := p.getELBClient(region)
